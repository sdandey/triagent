#!/usr/bin/env python3
"""Generate comprehensive Pull Request report for Omnia Data teams.

This script queries Azure DevOps for all PRs in the past 6 months,
organizes them by team, and generates an HTML report with visualizations.
"""
...
# Team definitions with their area paths
TEAMS = {
...
def get_work_item_team(work_item_id: int) -> str | None:
    """Get the team for a work item by querying its area path."""
    command = f'az boards work-item show --id {work_item_id} --org "{ORG_URL}" --output json'
    result = run_az_command(command)

    if result and "fields" in result:
        area_path = result["fields"].get("System.AreaPath", "")

        # Match area path to team
        for team_name, team_area_path in TEAMS.items():
            if area_path.startswith(team_area_path):
                return team_name
...
def parse_date(date_str: str | None) -> datetime | None:
    ...
    try:
        ...
        # Format: 2025-06-24T12:34:56.123Z
        return datetime.fromisoformat(date_str.replace("Z", "+00:00"))
    except (ValueError, AttributeError):
        return None


def get_pr_team(pr: dict[str, Any]) -> str:
    """Determine the team for a PR based on work items or creator."""
    # Try to get team from linked work items
    work_item_refs = pr.get("workItemRefs")

    if work_item_refs and isinstance(work_item_refs, list):
        for work_item_ref in work_item_refs:
            work_item_id = work_item_ref.get("id")
            if work_item_id:
                team = get_work_item_team(int(work_item_id))
                if team:
                    return team

    # If no work items or team not found, categorize as "Unknown"
    return "Unknown"
...
def analyze_prs(all_prs: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    daily_by_team: dict[str, dict[str, dict[str, int]]] = defaultdict(lambda: defaultdict(lambda: {"created": 0, "merged": 0}))
...
def generate_html_report(analysis: dict[str, Any], output_file: str) -> None:
    ...
    total_prs = sum(len(prs) for prs in team_prs.values())
    total_merged = sum(1 for prs in team_prs.values() for pr in prs if pr.get("status") == "completed")
...
<head>
    ...
    <title>Pull Request Report - Omnia Data Teams</title>
...
<body>
    <div class="container">
        ...
        <div class="summary">
            ...
            <div class="summary-card">
                <h3>Teams</h3>
                <div class="value">{len(team_prs)}</div>
            </div>
            <div class="summary-card">
                <h3>Merge Rate</h3>
                <div class="value">{(total_merged / total_prs * 100):.1f}%</div>
            </div>
        </div>
        ...
        for team_name in pod_teams_with_prs:
            ...
            html += f"""
                <div class="pod-section">
                    <h3>{team_name} <span class="badge badge-info">{len(prs)} PRs</span></h3>
                    <table>
                        <thead>
                            <tr>
                                <th>PR ID</th>
...
"""
            ...
            for pr in sorted_prs[:50]:  # Limit to 50 PRs per team for readability
                ...
                web_url = pr_url.replace("_apis/git/repositories", "_git") if pr_url else "#"
...
def main():
    ...
    print("Pull Request Report Generator", file=sys.stderr)
    print("=" * 80, file=sys.stderr)
    ...
    output_file = f"/Users/sdandey/Documents/code/triagent/pr_report_{END_DATE.strftime('%Y%m%d')}.html"
